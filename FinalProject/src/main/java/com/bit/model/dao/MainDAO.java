...
public interface MainDAO {
	...
	@Select("select userNo from user_info where email=#{email}")
	public int getUserNo(String email); // 유저 아이디 중복 체크
	...
	@Select("select email from user_info where email= #{email} and key = #{key} ")
	public String getEmailCheck(String email, String key); // 회원가입한 유저 이메일 확인
